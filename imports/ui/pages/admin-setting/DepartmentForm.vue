      ...
      <q-card style="width: 70%; max-width: 80vw">
        ...
        <q-card-actions align="right" class="bg-white text-teal">
          <div class="text-right q-gutter-sm">
            <q-btn v-if="$userIsInRole('insertDepartment') && !showId" color="primary" @click="submit"> Save </q-btn>
            <q-btn v-if="$userIsInRole('updateDepartment') && showId" color="primary" @click="submit"> Save </q-btn>
            <q-btn v-if="showId && $userIsInRole('removeDepartment')" color="red" @click="remove"> Remove </q-btn>
...
// const currentBranchId = store.getters['app/currentBranchId']
  ...
  const checkExist = (selector) => {
    return new Promise((resolve, reject) => {
      Meteor.call('checkDepartmentExist', { selector }, (err, res) => {
        if (err) {
          reject(err)
        } else {
          resolve(res)
  ...
  const insert = () => {
    ...
    Meteor.call('insertDepartment', form.value, (err, res) => {
      if (err) {
        console.log(err)
