    ...
    >
      <template #top>
        <div class="q-gutter-x-md">
          <q-btn color="primary" no-caps icon="add" @click.prevent="addNew" v-if="$userIsInRole('insertEmployeeType')">
            Add
          </q-btn>
        </div>
...
import EmployeeTypeFormVue from './EmployeeTypeForm.vue'
  const columns = [
    {
      name: 'name',
      label: 'Name',
  ...
  const fetchData = () => {
    ...
    Meteor.call('findEmployeeType', match, (err, res) => {
      ...
      if (err) {
        ...
        // this.list = res
        console.log('fetch EmployeeType success',res.data)
        data.value = res.data || []
