      ...
      <q-table v-model:pagination="pagination" bordered flat :rows="data" :columns="columns" :filter="filter"
        ...
        <template #body-cell-action="props" v-if="$userIsInRole(['acceptLeave'])">
          <q-td :props="props" v-if="props.row.status === 'active'">
            <q-btn dense class="q-mr-md" icon="done" color="primary"
              @click="updateStatus(props.row, status = 'accepted', acceptedById = currentUser)">
              <q-tooltip transition-show="flip-right" transition-hide="flip-left" class="bg-black" :offset="[10, 10]">
                accept leave
              </q-tooltip>
            </q-btn>
            <q-btn dense @click="updateStatus(props.row, status = 'canceled', acceptedById = currentUser)" icon="close"
              color="red">
              <q-tooltip transition-show="flip-right" transition-hide="flip-left" class="bg-black" :offset="[10, 10]">
                cancel leave
              </q-tooltip>
          ...
          <q-td :props="props" v-else disabled>
            <q-btn class="q-mr-md" color="grey">
